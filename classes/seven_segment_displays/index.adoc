...
During this lab we are going to extend on what we learned about sequential logic and memory and learn about clocked circuits and state machines. We will narrow the focus on what it means to be synchronous to an event as well as learn how to use memory to drive complex behaviors in computer systems.

== Clocks, Edges, and Flip-Flops Oh My!

In last week's lab, we covered making circuits only change their output when certain signals were high. This meant those circuits were sequential to whatever signal enabled them to change, usually a signal called `Store` or `Enable`. However, you can go even further, and we will in this lab. As an example, let's revisit the 8 bits of storage we wrote last lab, and take a look at some of its behavior:
