...
NOTE: Use the sensitivity list here. An important feature of the D-Latch is that it is only synchronous to the E signal.
...
The de/multiplexer blocks you have already written in previous labs, and this will just be an extension thereof, where we have a four input, 8 bit wide multiplexer, and a four output, 8 bit wide demultiplexer.
...
WARNING: There is only one new bit of Verilog syntax here -- the array syntax. See <<array_appendix>> below for more information.

NOTE: If you are having trouble, see <<scaffold_appendix>> for some scaffolding to help you get started.
